...
@SARKAUT.on(events.NewMessage(pattern="^[?!/]الرسائل المثبتة"))
async def get_pinned(event):
    chat_id = (str(event.chat_id)).replace("-100", "")

    Ok = await SARKAUT.get_messages(event.chat_id, ids=types.InputMessagePinned()) 
...
@SARKAUT.on(events.NewMessage(pattern="^[!?/]تثبيت ?(.*)"))
@is_admin
async def pin(event, perm):
    ...
    if "notify" in input_str:
       await SARKAUT.pin_message(event.chat_id, msg, notify=True)
       return
    await SARKAUT.pin_message(event.chat_id, msg)   

@SARKAUT.on(events.NewMessage(pattern="^[!?/]الغاء تثبيت ?(.*)"))
@is_admin
...
@SARKAUT.on(events.callbackquery.CallbackQuery(data="pins"))
async def _(event):
