...
btnn =[
    [Button.url("پشتگیری", url=f"t.me/{Config.SUPPORT}"), Button.url("که ناڵی بۆت", url=f"t.me/{Config.CHANNEL}")],
    ...
    if (
        ...
        if search == 0:
            ...
            if hm == 0:
                ...
                caption = f"- **بۆ ئەوەی زیاد بکرێت بۆ پلەی لیست »** `#{pos}`\n\n**🏷 ناونیشانەکە:** [{songname}]({url})\n**⏱ ماوه که ی:** `{duration}`\n🎧 **له لایه ن:** {from_user}"
                await botman.delete()
        ...
        if chat_id in QUEUE:
            ...
            caption = f"💡 ** زیادکراوە بۆ پلەی لیست**`#{pos}`\n\n**🏷 ناونیشانه که:** [{songname}]({link})\n**ئایدی گرووپ**: `{chat_id}`\n🎧 **له لایه ن**: {from_user}"
...
async def vplay(event):
    ...
    if replied and not replied.video and not replied.document:
        ...
        if chat_id in QUEUE:
            ...
            caption = f"💡 **ڤیدیۆکە لە چاتەکەدا پەخش کراوە** `#{pos}`\n\n**🏷 ناونیشانه که:** [{songname}]({link})\n**ئایدی گرووپ**: `{chat_id}`\n🎧 **لەسەر داواکاری ** {from_user}"
            await event.client.send_file(chat_id, ngantri, caption=caption, buttons=btnn)
            await razan.delete()
        else:
            if RESOLUSI == 360:
...
async def vc_skip(event, perm):
    ...
    if len(event.text.split()) < 2:
        ...
        DELQUE = "**باقیەکەی تر لە پلەی لیستەکە دەسڕدرێنەوە:**"
        if chat_id in QUEUE:
...
async def vc_resume(event, perm):
    ...
    if chat_id in QUEUE:
        ...
        await event.reply(event, "**هیچ شتێک بە هیچ شێوەیەک بەڕێوە ناچێت**")
