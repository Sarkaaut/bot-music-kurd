...
/omitted
بۆ بینینی ئەکاونتە سڕاوەکان لە چاتدا🗑️

/DeletedClean
سرێنه وه و ده رکردنی ئاکاونته سراوه کانی گرووپ⚠️
...
@R0R77.on(events.NewMessage(pattern="^[!?/]omitted ?(.*)"))
@is_admin
async def clean(event, perm):
    ...
    if "clean" not in input_str:
      ...
      async for user in event.client.iter_participants(event.chat_id):
            ...
            if user.deleted:
                deleted += 1
      if deleted > 0:
            stats = f".دۆزرایەوە **{deleted}** لێرەدا سڕاوەتەوە\
            \ بۆ دەرکردنیان بنێره `/ /DeletedClean`"
    ...
    async for user in event.client.iter_participants(event.chat_id):
        if user.deleted:
            try:
                ...
                await cleaning_zombies.edit("- ببورن لێرە مۆڵەتم نییە بۆ بلۆککردن👤")
                return
    ...
    if del_u > 0:
        stats = f"پاککراوەتەوە`{del_u}` لە ئەکاونتە سڕاوەکانەوە🚷"
